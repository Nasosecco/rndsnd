...
from PySide6.QtCore import Qt, QMimeData, QUrl, QTimer, QSize, QPoint, QThread, Signal, QElapsedTimer
    ...
    def run(self):
        if AI_AVAILABLE:
            ...
            try:
                ...
                self.log.emit("✅ AI Ready! Starting scan...")
            except Exception as e:
                self.log.emit(f"❌ AI Error: {e}")
                self.ai_model = None
        ...
        for i, path in enumerate(file_list):
            ...
            try:
                ...
                if count % 2 == 0 or count == total:
                    ...
                    self.log.emit(f"Analysis: {filename[:25]}... [{tags}]")
            except: pass
...
class RndSndApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("rndsnd v0.6.5")
    ...
    def setup_header(self):
        ...
        header.addWidget(QLabel("THEME:")); header.addWidget(self.theme_combo)
        self.main_layout.addLayout(header)

    def switch_theme(self, theme):
        # FIX: Light theme waveform color changed to #ff9800 (Orange)
        if theme == "Dark":
            self.setStyleSheet(DARK_STYLE); self.canvas_bg, self.wf_color, self.axis_color, self.cursor_color = '#1e1e1e', '#ff9800', 'white', 'white'
        else:
            self.setStyleSheet(COMMON_BUTTON_STYLE); self.canvas_bg, self.wf_color, self.axis_color, self.cursor_color = 'white', '#ff9800', 'black', 'black'
    ...
    def setup_archive_tab(self):
        ...
        lh = QHBoxLayout()
        # [EN] OPEN / REFRESH
        self.btn_load = QPushButton("OPEN"); self.btn_load.clicked.connect(self.open_load_dialog); self.btn_load.setStyleSheet("background-color: #6200ea; color: white;")
        self.btn_refresh = QPushButton("REFRESH"); self.btn_refresh.clicked.connect(self.refresh_current_library); self.btn_refresh.setStyleSheet("background-color: #2e7d32; color: white;")
        lh.addWidget(self.btn_load); lh.addWidget(self.btn_refresh); l_lyt.addLayout(lh)
        
        self.archive_status_lbl = QLabel("Ready."); self.archive_status_lbl.setStyleSheet("color: #aaa; font-style: italic;")
        ...
        self.search_input = QLineEdit(); self.search_input.setPlaceholderText("🔍 Search tags...")
        ...
        # [EN] Headers
        self.file_table.setHorizontalHeaderLabels(["Name", "Duration", "Size", "Date", "Tags"])
    ...
    def setup_mixer_tab(self):
        ...
        self.radio_tags = QRadioButton("Use filtered list (Search)"); self.radio_tags.setChecked(True)
        self.radio_chaos = QRadioButton("Full Chaos (Entire DB)")
    ...
    def open_load_dialog(self):
        f = QFileDialog.getExistingDirectory(self, "Select audio folder")
    ...
    def start_scan_process(self, folder):
        ...
        self.archive_status_lbl.setText("AI Analysis in progress...")
    ...
    def on_scan_finished(self, count):
        ...
        self.refresh_list(); self.archive_status_lbl.setText(f"✅ Completed: {count} files analyzed.")
    ...
    def generate_mix(self):
        ...
        self.status_lbl.setText(f"✅ Created: {fname}.{ext}")
...
if __name__ == "__main__":
    ...
    splash = None
    # FIX: Splash logic modified to force repaint and process events
    if os.path.exists(sp):
        ...
        if not pix.isNull():
            ...
            splash.show()
            splash.repaint() # FORCE REPAINT
            
            # Use QElapsedTimer to wait while keeping UI responsive
            timer = QElapsedTimer()
            timer.start()
            while timer.elapsed() < 2500: # 2.5 seconds
                app.processEvents()
                time.sleep(0.01)
